...
gaborgen = function(tilt, sf, phase) {
  var a, aspectratio, b, contrast, gab_x, gab_y, gridArray, i, j, m, multConst, preSinWave, ref, reso, sc, scaledM, sf_max, sf_min, sinWave, tilt_max, tilt_min, varScale, x, x_centered, x_factor, y, y_centered, y_factor;
  ...
  m = numeric.add(.5, numeric.mul(
    contrast, 
    numeric.transpose(
      numeric.mul(
        numeric.mul(multConst, numeric.exp(
          numeric.add(
            numeric.div(x_factor, varScale), numeric.div(y_factor, varScale)
          )
        )), 
      sinWave
    ))));
  ...
  const output = []
  // for (let i = 0; i < scaledM.length; i++){
  //   for (let j = 0; j < scaledM[0].length; j++){
  for (let i = 0; i < 400; i++){
    for (let j = 0; j < 400; j++){
      output.push(scaledM[i][j]) // R
      output.push(scaledM[i][j]) // G
      output.push(scaledM[i][j]) // B
      output.push(255) // alpha
    }
  }
  // console.log(output)
  return output
  // console.log(scaledM) // RGBA (A=255)
