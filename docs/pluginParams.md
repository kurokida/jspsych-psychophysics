...
|raf_func|function|null|**Advanced.** This function takes three arguments which are, in order, `trial`, `elapsed time in ms`. and `elapsed time in frame`. This function is called by the *requestAnimationFrame* method, and excuted synchronized with the refresh of the display. When you specify the *raf_func*, the *stimuli* is not necessarily specified. If you would like to draw stimuli using the canvas-drawing methods manually, the *raf_func* would be benefit. See, `demos/raf_func.html` and `demos/draw_two_images_repeatedly.html`.|
