...
|upper_prompt|string|null|This string can contain HTML markup. Any content here will be displayed above the canvas.|
|lower_prompt|string|null|This is the same as `prompt`. Any content here will be displayed below the canvas.|
...
|button_html|function|(choice: string, choice_index: number)=>`<button class="jspsych-btn">${choice}</button>;|See [the html-button-response document](https://www.jspsych.org/latest/plugins/html-button-response/).|
|button_layout|string|'grid'|See [the html-button-response document](https://www.jspsych.org/latest/plugins/html-button-response/).|
|grid_rows|number|1|See [the html-button-response document](https://www.jspsych.org/latest/plugins/html-button-response/).|
|grid_columns|number|null|See [the html-button-response document](https://www.jspsych.org/latest/plugins/html-button-response/).|
...
|touchstart_func|function|null|This is the event handler of the touchstart. See [the touch events page on MDN Web Docs](https://developer.mozilla.org/en-US/docs/Web/API/Touch_events). |
|touchmove_func|function|null|This is the event handler of the touchmove.|
|touchend_func|function|null|This is the event handler of the touchend.|
|touchcancel_func|function|null|This is the event handler of the touchcancel.|
...
|click_x/click_y|numeric|Horizontal/Vertical clicked position. The origin is **the center** of the canvas, and the unit is pixels. Positive values indicate positions to the right (horizontal) and downward (vertical).|
|center_x/center_y|numeric|Horizontal/Vertical position of the canvas center. The origin is located at **the top-left** of the canvas, and the unit is pixels. Positive values indicate positions to the right and downward.|
